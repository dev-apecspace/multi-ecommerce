...
**Implementation**:
1. **Created**: `lib/loading-context.tsx`
   - LoadingProvider component for state management
   - useLoading hook to access/control loading state

2. **Created**: `components/global-loading.tsx`
   - Global loading overlay with spinner
   - Displays full-screen overlay when isLoading is true

3. **Updated**: `components/providers.tsx`
   - Wrapped app with LoadingProvider
   - Added GlobalLoading component to root layout
...
- Loading box: White rounded container with spinner and text
- Spinner color: Orange (`text-orange-500`) to match brand
