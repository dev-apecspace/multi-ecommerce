...
export default function ClientReturnsPage() {
  ...
  const pendingReturns = returns.filter((r) => r.status === "pending" && r.returnType === "return")
  const approvedReturns = returns.filter((r) => r.status === "approved" && r.returnType === "return")
  const shippedReturns = returns.filter((r) => r.status === "shipped" && r.returnType === "return")
  const completedReturns = returns.filter((r) => r.status === "completed" && r.returnType === "return")
  const rejectedReturns = returns.filter((r) => r.status === "rejected" && r.returnType === "return")
  ...
  return (
    <main className="container-viewport py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Quản lý các yêu cầu trả hàng của bạn</p>
      </div>
