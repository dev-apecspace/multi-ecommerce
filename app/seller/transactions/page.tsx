...
export default function SellerTransactionsPage() {
  ...
  }, [user?.vendorId, dateRange.start, dateRange.end, paymentStatus, pagination.page, pagination.limit])

  const fetchTransactions = async () => {
    try {
      setLoading(true)
      const url = new URL('/api/seller/transactions', window.location.origin)
  ...
  const getStatusBadge = (tx: SellerTransaction) => {
    ...
    if (tx.isEligibleRevenue) {
      return <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200"><CheckCircle2 className="h-3.5 w-3.5 mr-1" /> Ghi nhận doanh thu</Badge>
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2">
            ...
            Giao dịch & Doanh thu Cửa hàng
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Quản lý dòng tiền bán hàng, trạng thái thanh toán và doanh thu thực nhận
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card className="border-emerald-200/60 dark:border-emerald-900/40 bg-gradient-to-br from-emerald-50/50 to-transparent dark:from-emerald-950/10">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Doanh thu ghi nhận</p>
            ...
            <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-2 font-medium">
              Từ {summary.paidTransactionsCount} giao dịch thành công
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-amber-200/60 dark:border-amber-900/40 bg-gradient-to-br from-amber-50/50 to-transparent dark:from-amber-950/10">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Doanh thu chờ ghi nhận</p>
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 mt-2 font-medium">
              Đơn hàng đang xử lý hoặc đang vận chuyển
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-red-200/60 dark:border-red-900/40 bg-gradient-to-br from-red-50/50 to-transparent dark:from-red-950/10">
          <CardContent className="p-5">
            ...
            <p className="text-xs text-red-700 dark:text-red-400 mt-2 font-medium">
              Không tính vào tổng doanh thu cửa hàng
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Tất cả các đơn hàng phát sinh của Shop
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mb-6">
        <CardContent className="p-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-xs font-bold text-muted-foreground uppercase">Trạng thái ghi nhận:</span>
            <select
              value={paymentStatus}
            ...
            >
              ...
              <option value="paid">Đã ghi nhận doanh thu</option>
              <option value="pending">Chờ ghi nhận (Đang giao/Xử lý)</option>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-bold flex items-center justify-between">
            <span>Danh sách giao dịch đơn bán</span>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-12 text-center text-muted-foreground">Chưa có giao dịch bán hàng nào.</div>
          ) : (
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-muted/50 border-y border-border text-xs text-muted-foreground uppercase font-semibold">
                  <tr>
                    ...
                    <th className="py-3 px-4">Trạng thái doanh thu</th>
                    ...
                    <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-3.5 px-4 max-w-xs">
                        <p className="font-medium truncate">
                          {tx.OrderItem?.map(i => i.Product?.name).filter(Boolean).join(', ') || 'Sản phẩm'}
                        </p>
                        <p className="text-xs text-muted-foreground">({tx.OrderItem?.length || 0} sản phẩm)</p>
                      </td>
          ...
          <DialogHeader className="border-b pb-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pr-6 min-w-0">
              <DialogTitle className="text-lg sm:text-xl font-bold truncate break-all min-w-0">
                Chi tiết giao dịch bán hàng #{selectedTx?.orderNumber || selectedTx?.id}
              </DialogTitle>
            ...
            <div className="space-y-4 pt-2 min-w-0">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 text-sm bg-muted/40 p-3 sm:p-4 rounded-xl border min-w-0">
                <div className="min-w-0">
                  <p className="text-xs text-muted-foreground font-medium">Khách hàng mua</p>
                ...
                <div className="min-w-0">
                  <p className="text-xs text-muted-foreground font-medium">Giá trị đơn hàng</p>
                ...
                <div className="min-w-0">
                  <p className="text-xs text-muted-foreground font-medium">Ngày giao dịch</p>
              ...
              {/* Items List */}
              <div className="min-w-0">
                <h4 className="font-semibold text-sm mb-2">Chi tiết sản phẩm bán</h4>
              ...
              {selectedTx.paymentProofUrl && (
                <div className="pt-2 border-t min-w-0">
                  <h4 className="font-semibold text-sm mb-2">Ủy nhiệm chi / Ảnh chứng từ thanh toán của khách</h4>
